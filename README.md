...
> **보고서 수정사항**
> 초기 보고서에서는 수명 비교 대상을 `Q1`, `Q4` 셀 중심으로 해석했지만, 현재 보고서에서는 이를 `cycle_life <= 500`, `501-999`, `>=1000` 기준으로 수정했다.
> 변경 이유는 `Q1`, `Q4`가 배치 내부 상대 구간이라 배치가 달라지면 기준 자체가 함께 변해 해석 일관성이 떨어지기 때문이다. 반면 `500`, `1000` 기준은 단수명/장수명 셀을 절대 기준으로 더 직관적으로 구분할 수 있어 batch 간 분포 차이와 모델의 실제 예측 난이도를 설명하는 데 더 적합하다.
